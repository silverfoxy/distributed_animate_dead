...
foreach($covered_files as $key => $entry) {
    $lim_covered_files[] = str_replace('/var/www/html/hotcrp/', '', explode(': ', $entry['file_name'])[0]);
}
...
foreach ($ad_line_coverage as $line) {
    if (strpos($line, ': ') !== false) {
        $filename = str_replace('/home/ubuntu/debloating_templates/app/', '', explode(': ', $line)[0]);
